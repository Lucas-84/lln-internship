...
LL hel_init_data(hel_data_t* data, hel_preprocessing_t* preprocessing, double** score_mat_init, LL* key_init){
...
LL hel_init_enum_input(hel_enum_input_t* enum_input, hel_preprocessing_t* preprocessing, hel_data_t* data, hel_histo_t* histo , ZZ bound_start, ZZ bound_end, LL test_key_boolean ,unsigned char** pt_ct, LL enumerate_to_real_key_rank, LL up_to_bound_bool){
	...
	enum_input->index_list = (LL**) malloc(preprocessing->updated_nb_subkey*sizeof(LL*));
	for (i = 0;  i < preprocessing->updated_nb_subkey ; i++){
		enum_input->index_list[i] = (LL*) malloc((histo->nb_bins+1)*sizeof(LL));
	}
	enum_input->index_list_alloc_bool = 1;

	enum_input->convolution_order = (LL*) malloc(preprocessing->updated_nb_subkey* sizeof(LL));
	enum_input->convolution_order_alloc_bool = 1;


	enum_input->key_factorization = (LL**) malloc(preprocessing->updated_nb_subkey*sizeof(LL*));
	for (i = 0;i  < preprocessing->updated_nb_subkey ; i++){
		enum_input-> key_factorization[i] = (LL*) malloc( (preprocessing->updated_nb_key_value[i]+1)*sizeof(LL));
	}
	...
	*(enum_input->bound_end) = bound_end;
	...
	if (  !data->log_probas_real_key_alloc_bool && enumerate_to_real_key_rank ){
		...
		enum_input->real_key = (LL*) malloc(preprocessing->updated_nb_subkey*sizeof(LL));
		for (i = 0; i<  preprocessing->updated_nb_subkey ; i++){
