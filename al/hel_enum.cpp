...
LL*** convert_list_into_array( hel_int_list_t*** key_list, LL nb_bins, LL updated_nb_subkey){
	...
	LL*** ret = (LL***) malloc( updated_nb_subkey*sizeof(LL**));
	...
	for (i = 0 ; i < updated_nb_subkey ; i++){
		...
		ret[i] = (LL**) malloc( nb_bins*sizeof(LL*));
		...
		for (j = 0; j < nb_bins ; j++){
			...
			if ( nb_elem != 0){
				...
				ret[i][j] = (LL*) malloc( (nb_elem+1)*sizeof(LL));
				curr = key_list[i][j];
...
//convert all the ZZX hists used for the recursive iteration into  LL* binary histograms which contains 0 if bin is empty, 1 otherwise.
...
LL** get_binary_hist_from_big_hist(ZZX* hists, LL* big_hist_size,LL* convolution_order, LL updated_nb_subkey){
	...
	LL** binary_hists = (LL**) malloc( updated_nb_subkey*sizeof(LL*));

	ZZ zz_zero;
	zz_zero = 0;

	for (i = updated_nb_subkey-1 ; i >= 1; i--){
		binary_hists[i] = (LL*) calloc( deg(hists[updated_nb_subkey+i-1]) +1, sizeof(LL) );
	...
	binary_hists[0] = (LL*) calloc(  deg(hists[convolution_order[1]]) +1 ,sizeof(LL));
...
*/
LL set_enum_input(hel_enum_input_t* enum_input, hel_enum_info_t* enum_info , hel_preprocessing_t* preprocessing, hel_histo_t* histo){
	...
	if(enum_input->enumerate_to_real_key_rank && ( *(enum_input->bound_end) < *(enum_info->nb_key_enum_end) )  ){
		...
		enum_input->binary_hists_size = (LL*) malloc( preprocessing->updated_nb_subkey * sizeof(LL));
...
LL print_file_key_original(LL** current_key_facto,LL test_key_boolean, LL updated_nb_subkey, LL merge_value, unsigned char** pt_ct){
	...
	if (test_key_boolean){ //test key using the provided AES (not optimized)
		...
		indexes = (LL*) calloc( updated_nb_subkey,sizeof(LL));
		current_index = 0;
		incremente_to_do = 1;
...
void decompose_bin(LL current_small_hist, LL current_index, hel_preprocessing_t* preprocessing, hel_enum_input_t* enum_input, LL* found_key){
	...
	LL current_index_small_hist;
	LL current_index_big_hist_m_1;
...
void start_recursive_enumeration( hel_preprocessing_t* preprocessing, hel_histo_t* histo, hel_enum_input_t* enum_input, hel_enum_info_t* enum_info){
	...
	LL i,j;
	LL current_small_hist = preprocessing->updated_nb_subkey-1;

    //variable used to prLL the current part of enumeration
	RR rr_two = conv<RR>("2");
    RR current_power= conv<RR>(  log2(conv<double>(*(enum_input->bound_start)))  );


	LL total_bin_nb = (LL) deg(histo->hists[preprocessing->updated_nb_subkey*2-2])+1;
	...
	*(enum_info->nb_key_enum_found_key) = 0;
	...
	if(enum_info->found_key_boolean){
		...
		*(enum_info->nb_key_enum_found_key_bound_max) = *(enum_info->nb_key_enum_found_key);
		...
		if ( enum_info->bin_found_key_bound_min > (LL) deg(histo->hists[preprocessing->updated_nb_subkey*2-2]) ){
