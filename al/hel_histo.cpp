...
LL find_real_key_bin(hel_histo_t* histo, hel_data_t* data, hel_preprocessing_t* preprocessing){
	...
	for (i = 0; i < preprocessing->updated_nb_subkey ; i++){
		...
		if ( tmp_score < (*(histo->width)) ){
			...
			tmp = (LL) conv<long>(tmp_score/ (*(histo->width)) );
		}
...
void compute_single_histogram(hel_algo_mode_t algo_mode, ZZX* hist, LL subkey_number, hel_data_t* data, hel_histo_t* histo, hel_enum_input_t* enum_input, hel_preprocessing_t* preprocess){
	...
	LL nb_test = preprocess->updated_nb_key_value[subkey_number];
	...
	for ( i = 0; i < nb_test ; i++){
		...
		if (tmp_score < (*(histo->width)) ){
			...
			target_bin = (LL) conv<long>(tmp_score/(*(histo->width)));
...
//for qsort decreasing order LL array
...
void search_convo_order(LL** index, LL* convo_order, LL updated_nb_subkey){
	...
	LL* tmp_sort_nb_bin = (LL*) malloc( updated_nb_subkey*sizeof(LL));
...
ZZX* compute_histograms_procedure(hel_algo_mode_t algo_mode, hel_preprocessing_t* preprocessing, hel_data_t* data, hel_histo_t* histo, hel_enum_input_t* enum_input){
	...
	RR rr_nb_bins = conv<RR>((int)histo->nb_bins);
	*(histo->width) = (rr_max + rr_max/rr_nb_bins) / rr_nb_bins; //small margin !

	ZZX* hists = new ZZX[preprocessing->updated_nb_subkey*2]; // used to be x*2-1
	histo->hists_alloc_bool = 1;

//	printf("size = %lld -> %lld\n", preprocessing->updated_nb_key, preprocessing->updated_nb_subkey*2-1);

	//compute the initial histograms
	for (i = 0;  i < preprocessing->updated_nb_subkey ; i++){
		hists[i].SetLength(histo->nb_bins);
		compute_single_histogram( algo_mode, &(hists[i]), i, data, histo,  enum_input,  preprocessing);
	}
	puts("and then");
	...
	if(algo_mode == ENUM){
		...
		for ( i = 0 ; i  < preprocessing->updated_nb_subkey ; i++){
			qsort(enum_input->index_list[i]+1, enum_input->index_list[i][0], sizeof(LL), cmp_array);
		}
...
void get_real_key_info(hel_real_key_info_t* real_key_info, hel_preprocessing_t* preprocessing, hel_histo_t* histo, hel_data_t* data ){
	...
	*(real_key_info->bound_min) = 0;
	...
	LL last_bin_index = (LL) deg(histo->hists[preprocessing->updated_nb_subkey*2-2]);
